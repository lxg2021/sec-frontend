...
export function ScanScheduleForm({
  ...
  const handleChange = (updates: Partial<typeof schedule>) => {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{schedule.interval_hours ?? 24} 小时</span>
          </div>
          <Slider
            id="interval_hours"
            min={1}
        ...
        <div className="space-y-2">
          <Label htmlFor="specific_time">特定执行时间（可选）</Label>
          <Input
            id="specific_time"
            type="time"
            value={schedule.specific_time ?? ""}
            onChange={(e) => handleChange({ specific_time: e.target.value || undefined })}
            placeholder="选择执行时间"
            disabled={disabled}
          />
          <p className="text-xs text-muted-foreground">设置后，任务将在指定时间点执行</p>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{schedule.random_delay_minutes ?? 0} 分钟</span>
          </div>
          <Slider
            id="random_delay"
            min={0}
          ...
          <p className="text-xs text-muted-foreground">在计划执行时间上添加随机延迟，避免同一时刻触发大量任务。</p>
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange({
                    retry_interval_minutes: Math.max(1, Number(e.target.value)),
                  })
                }
        ...
        {showStartup ? (
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              <Label htmlFor="scan_on_startup" className="flex items-center gap-2 cursor-pointer">
                <Play className="size-4 text-muted-foreground" />
                启动时执行扫描
              </Label>
              <p className="text-xs text-muted-foreground">程序启动后立即执行一次扫描任务。</p>
            </div>
            <Switch
              id="scan_on_startup"
