...
function oneofFieldValue(item: BackendObject, ...keys: string[]) {
  const direct = fieldValue(item, ...keys)
  if (direct !== undefined) return direct
  const detail = objectValue(fieldValue(item, "detail", "Detail"))
  return fieldValue(detail, ...keys)
}
...
function normalizeWhitelistHitDetail(
  ...
  return {
    ...
    updated_at: stringValue(fieldValue(item, "updated_at", "updatedAt", "UpdatedAt")),
    hash: normalizeWhitelistHashDetail(
      fieldValue(item, "hash", "Hash") ?? fieldValue(detail, "hash", "Hash"),
    ),
    domain: normalizeWhitelistDomainDetail(
      fieldValue(item, "domain", "Domain") ?? fieldValue(detail, "domain", "Domain"),
    ),
    ip: normalizeWhitelistIPDetail(
      fieldValue(item, "ip", "Ip") ?? fieldValue(detail, "ip", "Ip"),
    ),
    certificate: normalizeWhitelistCertificateDetail(
      fieldValue(item, "certificate", "Certificate") ??
        fieldValue(detail, "certificate", "Certificate"),
    ),
    popularity: normalizeWhitelistPopularityDetail(
      fieldValue(item, "popularity", "Popularity") ??
        fieldValue(detail, "popularity", "Popularity"),
    ),
  }
}
...
function normalizeVerificationSourceDetail(
  ...
  const whitelist = normalizeWhitelistHitDetail(
    oneofFieldValue(item, "whitelist", "whitelistDetail", "Whitelist"),
  )
  ...
  const iocEntry = normalizeIocEntryHitDetail(
    oneofFieldValue(item, "ioc_entry", "iocEntry", "IocEntry"),
  )
