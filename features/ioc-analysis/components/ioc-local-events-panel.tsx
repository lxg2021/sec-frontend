      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-xs text-slate-500">
            {t("lookback", { days: defaultLookbackDays })}
            <code className="ml-1 rounded bg-white px-1.5 py-0.5 text-slate-700">{result.source || currentValue || "-"}</code>
          </p>
        </div>
        {onRefresh ? (
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 rounded-md border-slate-200 bg-white"
            disabled={loading}
            onClick={onRefresh}
          >
