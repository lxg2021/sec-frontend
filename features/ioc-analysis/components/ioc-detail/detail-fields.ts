...
const DETAIL_FIELD_LABELS: LocalizedDetailMap = {
  "zh-CN": {
    ...
    categories: "分类",
    certificate_thumbprint: "证书指纹",
    cert_thumbprint: "证书指纹",
    cidr_prefix: "CIDR前缀",
    cloud_provider: "云厂商",
    comment: "备注",
    confidence: "置信度",
    ...
    file_format: "文件格式",
    file_type: "文件类型",
    file_type_guess: "文件类型猜测",
    file_type_mime: "MIME类型",
    file_name: "文件名",
    file_size: "文件大小",
    filename: "文件名",
    gimphash: "Gimphash",
  ...
  en: {
    ...
    feed_names: "Feed",
    file_arch: "File architecture",
    file_format: "File format",
    file_type: "File type",
    file_type_guess: "File type guess",
    ...
    rank: "Rank",
    rdata: "DNS record value",
    reason: "Reason",
    reasons: "Reasons",
    reference: "Reference",
    db: "Database",
    parents: "Parent count",
    parent_source_url: "Parent source link",
...
function isHiddenDetailField(column: string) {
  const key = detailFieldKey(column)
  const rawSegments = column.trim().toLowerCase().split(/[.[\]]+/).filter(Boolean)
  const leafKey = detailFieldKey(rawSegments[rawSegments.length - 1] || key)
