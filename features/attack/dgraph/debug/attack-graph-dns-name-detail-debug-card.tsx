...
const DNS_NAME_DETAIL_DEBUG_NODE: AttackGraphNodeModel = {
  id: "dns:tenant-1:update.example.test",
  key: "dns:tenant-1:update.example.test",
  entityType: "DnsName",
  displayName: "update.example.test",
  presentationKind: getAttackGraphNodePresentationKind("DnsName"),
  properties: {
...
export function AttackGraphDnsNameDetailDebugCard() {
  return (
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <CardTitle className="text-base font-semibold text-slate-900">
              DnsName Node Detail Debug
            </CardTitle>
            <div className="mt-1 text-xs text-slate-500">
              DnsName graph payload rendered by the detail config.
            </div>
          </div>
        </div>
      </CardHeader>
      <CardContent className="relative min-h-[700px] bg-slate-50 p-4">
        <TooltipProvider delayDuration={180}>
          <AttackGraphDetailCard
            item={{ kind: "node", node: DNS_NAME_DETAIL_DEBUG_NODE }}
