...
test("approve logic workbench uses a light split panel and supports double-click rename", () => {
  ...
  assert.match(pageSource, /\{t\("addCompany"\)\}/)
  assert.match(pageSource, /\{t\("downloadTemplate"\)\}/)
  assert.match(pageSource, /t\("sourceBackend"\)/)
  assert.match(pageSource, /\{t\("importTitle"\)\}/)
  assert.match(pageSource, /\{t\("importDescription"\)\}/)
...
test("logic tree exposes efficient tree editing interactions", () => {
  ...
  assert.match(treeSource, /t\("currentPath"\)/)
  assert.match(treeSource, /t\("expandAll"\)/)
  assert.match(treeSource, /t\("collapseAll"\)/)
